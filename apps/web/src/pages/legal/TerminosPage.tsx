...
export function TerminosPage() {
  ...
  const ciudad = config?.ciudad ?? 'tu ciudad'
  const provincia = config?.provincia
  const pais = config?.pais
  const fw = legalFor(pais)
  const legal = config?.legal
  const razonSocial = legal?.razonSocial
  const taxIdLabel = legal?.taxIdLabel || fw.taxIdLabel
  const ubicacion = [ciudad, provincia, pais].filter(Boolean).join(', ')
  const precioLabel = config?.precioMensual ? formatMoney(config.precioMensual) : 'el precio del plan vigente'
  const jurisdiccion =
    legal?.jurisdiccion ||
    `los tribunales competentes de ${[ciudad, provincia].filter(Boolean).join(', ')}`
    ...
    <div className="mx-auto flex w-full max-w-2xl flex-col gap-5 px-4 pt-6 pb-12 sm:px-6 sm:pt-10">
      ...
      <article className="prose-sm flex flex-col gap-4 text-sm leading-relaxed text-neutral-700">
        <Section title="1. Quiénes somos · Responsable">
          <p>
            <strong>{appNombre}</strong> es una plataforma de descuentos vecinales que opera
            en {ubicacion}. Conecta vecinos con comercios adheridos para acceder a descuentos y
            promociones.
          </p>
          <p>
            <strong>Responsable del servicio y del tratamiento de datos personales</strong>:
          </p>
          <ul>
            {razonSocial && (
              <li>
                <strong>Nombre / Razón social:</strong> {razonSocial}
              </li>
            )}
            {legal?.taxId && (
              <li>
        ...
        <Section title="3. Cuenta de comercio">
          ...
          <ul>
            ...
            <li>
              Pagar la suscripción mensual de <strong>{precioLabel}</strong> (sin permanencia, sin
              cargos adicionales) por adelantado vía Mercado Pago.
            </li>
            <li>
              Mantener actualizada su información (horarios, dirección, datos fiscales para
              facturación).
            </li>
            <li>No publicar promociones engañosas o que no se respeten en el local físico.</li>
          </ul>
        </Section>
        ...
        <Section title="4. Cuenta de vecino">
          ...
          <ul>
            ...
            <li>Brindar información veraz (identificación, email, WhatsApp).</li>
        ...
        <Section title="5. Plan y pagos">
          <p>
            El plan estándar tiene un costo final de <strong>{precioLabel} por mes</strong> (sin
            cargos por permanencia). La suscripción es mensual y se renueva automáticamente vía
            Mercado Pago hasta que el comercio la cancele desde su panel.
          </p>
          {fw.esArgentina ? (
            <p>
              En esta primera etapa, el responsable opera bajo el{' '}
              <strong>Régimen Simplificado para Pequeños Contribuyentes (Monotributo)</strong> y
              emite <strong>factura C</strong> por cada período cobrado. La factura se envía al email
              registrado dentro de las 48hs de confirmado el pago. Cuando el servicio escale a una
              forma societaria (S.A.S. o equivalente), pasaremos a emitir factura A o B según la
              condición fiscal del comercio adherido, sin cambios en el precio público.
            </p>
          ) : (
            <p>
              Por cada período cobrado emitimos el comprobante fiscal correspondiente según la
              normativa vigente en {pais ?? 'el país de operación'}, enviado al email registrado
        ...
        <Section title="6. Derecho de arrepentimiento">
          <p>
            En cumplimiento de <strong>{fw.consumerLaw}</strong>, el comercio tiene{' '}
            <strong>10 días corridos desde el alta</strong> para arrepentirse y solicitar el
            reembolso completo del primer cobro. Después del día 10, las cancelaciones terminan al
            final del período mensual ya pagado, sin reembolso prorrateado.
          </p>
        </Section>

        <Section title="7. Suspensión y cancelación">
          <p>
            El responsable de {appNombre} puede suspender o cancelar la cuenta de un comercio en
            caso de:
          </p>
          <ul>
            ...
            <li>Violación de las leyes aplicables.</li>
          </ul>
          <p>
        ...
        <Section title="8. Responsabilidad">
          <p>
            {appNombre} es un <strong>intermediario tecnológico</strong>. La relación contractual
            del descuento es entre el comercio y el vecino. El responsable de {appNombre} no es
        ...
        <Section title="9. Datos personales">
          ...
          <p>
            ...
            comprometen a no compartirlos con terceros y a respetar <strong>{fw.dataLaw}</strong>.
          </p>
        </Section>

        <Section title="10. Propiedad intelectual">
          <p>
            La plataforma, su código, diseño, la marca "{appNombre}" y sus contenidos son propiedad
        ...
        <Section title="12. Jurisdicción">
          <p>
            Cualquier conflicto se resolverá ante {jurisdiccion}, renunciando a todo otro fuero.
          </p>
        </Section>
