    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-start sm:justify-end sm:p-4">
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          <AlertsContent onOpenCoupon={onOpenCoupon} />
        </div>
      </div>
    </div>,
    document.body,
...
/** Cuerpo de alertas (toggle de push + feed + preferencias). Reutilizado por el
 *  panel (modal del header) y por la página /alertas. */
export function AlertsContent({ onOpenCoupon }: { onOpenCoupon: (id: string) => void }) {
  const { feed, categories } = useAlerts()
